...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!empty($first_name) && !empty($last_name) && !empty($email) && !empty($rfidtag) && !empty($idcard) && !empty($password) && !is_numeric($first_name) && !is_numeric($last_name)) {
        ...
        $query = "INSERT INTO users2 (user_id, first_name, last_name, password, rfidtag, email, idcard, phonenumber) VALUES ('$user_id', '$first_name', '$last_name', '$hashed_password', '$rfidtag', '$encrypted_email', '$idcard', '$phonenumber')";
